...
export const pharmafetchByCity = (city: string) =>store.dispatch({
    ...
    path : 'pharma',
    payload : {
        query:`query{fetchTwentyByCity(city: "${city}"){_id account{name}views contact{address{street city}tele{fix}}}}`
    }
})
export const pharmafetchAllCity = (city: string) =>store.dispatch({
    ...
    path : 'pharma',
    payload : {
        query:`query{fetchAllByCity(city: "${city}"){_id account{name}views contact{address{street city}tele{fix}}}}`
    }
})
export const pharmafetchDetails = (name: string) =>store.dispatch({
    ...
    path : 'pharma',
    payload : {
        query:`query{fetchPharmaById(id:"${name}"){_id account {name}views contact{address{street city}tele{fix}}}}`
    }
})
export const pharmaSearchByName = (name: string) =>store.dispatch({
    ...
    path : 'pharma',
    payload : {
        query:`query{searchPharmaByName(name:"${name}"){_id account{name}views contact{address{city}}}}`
    }
})